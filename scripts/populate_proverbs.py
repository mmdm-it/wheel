#!/usr/bin/env python3
"""
Populate Proverbs (31 chapters, ~915 verses) with all 9 languages.
"""
...
CHAPTERS_DIR = PROJECT_ROOT / "data" / "gutenberg" / "chapters" / "PROV"
...
# Proverbs chapter files (31 chapters)
# NAB: __PK0.HTM (ch1) - K0 in base36 = 20*36+0 = 720
# Spanish: __PLJ.HTM (ch1) - LJ = 21*36+19 = 775
# Italian: __PJ6.HTM (ch1) - J6 = 19*36+6 = 690
NAB_PROV = generate_base36_files("K0", NUM_CHAPTERS)
SPANISH_PROV = generate_base36_files("LJ", NUM_CHAPTERS)
ITALIAN_PROV = generate_base36_files("J6", NUM_CHAPTERS)
...
def main():
    print("=" * 60)
    print("Populating Proverbs with 9 Languages")
    print("=" * 60)
    ...
    # Hebrew WLC
    print("\n  Parsing Hebrew (WLC)...")
    all_translations['WLC'] = parse_hebrew_wlc(SOURCES_DIR / "hebrew" / "wlc" / "Prov.xml", "Prov")
    print(f"  ✅ Hebrew WLC: {sum(len(v) for v in all_translations['WLC'].values())} verses" if all_translations['WLC'] else "  ❌ Not found")
    
    # Greek LXX
    print("\n  Parsing Greek (LXX)...")
    all_translations['LXX'] = parse_greek_lxx(SOURCES_DIR / "greek" / "septuagint" / "19.Pro.txt")
    print(f"  ✅ Greek LXX: {sum(len(v) for v in all_translations['LXX'].values())} verses" if all_translations['LXX'] else "  ❌ Not found")
    
    # Latin Vulgate
    print("\n  Parsing Latin (VUL)...")
    all_translations['VUL'] = parse_latin_vulgate(SOURCES_DIR / "latin" / "clementine" / "src" / "utf8" / "Pr.lat")
    print(f"  ✅ Latin VUL: {sum(len(v) for v in all_translations['VUL'].values())} verses" if all_translations['VUL'] else "  ❌ Not found")
    
    # Russian Synodal
    print("\n  Parsing Russian (SYN)...")
    all_translations['SYN'] = parse_russian_synodal(SOURCES_DIR / "russian" / "synodal-77books" / "Синодальный перевод - 77 книг - txt" / "23_prit.txt")
    print(f"  ✅ Russian SYN: {sum(len(v) for v in all_translations['SYN'].values())} verses" if all_translations['SYN'] else "  ❌ Not found")
    
    # French néo-Crampon
    print("\n  Parsing French (NEO)...")
    all_translations['NEO'] = parse_french_usfm(SOURCES_DIR / "french" / "neo-crampon" / "21-PROfrancl.usfm")
    ...
    nab_files = {ch: nab_dir / fname for ch, fname in NAB_PROV.items()}
    ...
    spanish_files = {ch: spanish_dir / fname for ch, fname in SPANISH_PROV.items()}
    ...
    italian_files = {ch: italian_dir / fname for ch, fname in ITALIAN_PROV.items()}
    ...
    print(f"\n✅ Proverbs complete! {total_updates} translations added.")
