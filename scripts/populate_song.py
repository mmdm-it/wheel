#!/usr/bin/env python3
"""
Populate Song of Songs (8 chapters, ~117 verses) with all 9 languages.
"""
...
CHAPTERS_DIR = PROJECT_ROOT / "data" / "gutenberg" / "chapters" / "CANT"
...
# Song of Songs chapter files (8 chapters)
# NAB: __PL9.HTM (ch1) - L9 in base36 = 21*36+9 = 765
# Spanish: __PMI.HTM (ch1) - MI = 22*36+18 = 810
# Italian: __PKD.HTM (ch1) - KD = 20*36+13 = 733
NAB_SONG = generate_base36_files("L9", NUM_CHAPTERS)
SPANISH_SONG = generate_base36_files("MI", NUM_CHAPTERS)
ITALIAN_SONG = generate_base36_files("KD", NUM_CHAPTERS)
...
def main():
    print("=" * 60)
    print("Populating Song of Songs with 9 Languages")
    print("=" * 60)
    ...
    # Hebrew WLC
    print("\n  Parsing Hebrew (WLC)...")
    all_translations['WLC'] = parse_hebrew_wlc(SOURCES_DIR / "hebrew" / "wlc" / "Song.xml", "Song")
    print(f"  ✅ Hebrew WLC: {sum(len(v) for v in all_translations['WLC'].values())} verses" if all_translations['WLC'] else "  ❌ Not found")
    
    # Greek LXX
    print("\n  Parsing Greek (LXX)...")
    all_translations['LXX'] = parse_greek_lxx(SOURCES_DIR / "greek" / "septuagint" / "21.Sol.txt")
    print(f"  ✅ Greek LXX: {sum(len(v) for v in all_translations['LXX'].values())} verses" if all_translations['LXX'] else "  ❌ Not found")
    
    # Latin Vulgate
    print("\n  Parsing Latin (VUL)...")
    all_translations['VUL'] = parse_latin_vulgate(SOURCES_DIR / "latin" / "clementine" / "src" / "utf8" / "Ct.lat")
    print(f"  ✅ Latin VUL: {sum(len(v) for v in all_translations['VUL'].values())} verses" if all_translations['VUL'] else "  ❌ Not found")
    
    # Russian Synodal
    print("\n  Parsing Russian (SYN)...")
    all_translations['SYN'] = parse_russian_synodal(SOURCES_DIR / "russian" / "synodal-77books" / "Синодальный перевод - 77 книг - txt" / "25_pesn.txt")
    print(f"  ✅ Russian SYN: {sum(len(v) for v in all_translations['SYN'].values())} verses" if all_translations['SYN'] else "  ❌ Not found")
    
    # French néo-Crampon
    print("\n  Parsing French (NEO)...")
    all_translations['NEO'] = parse_french_usfm(SOURCES_DIR / "french" / "neo-crampon" / "23-SNGfrancl.usfm")
    ...
    nab_files = {ch: nab_dir / fname for ch, fname in NAB_SONG.items()}
    ...
    spanish_files = {ch: spanish_dir / fname for ch, fname in SPANISH_SONG.items()}
    ...
    italian_files = {ch: italian_dir / fname for ch, fname in ITALIAN_SONG.items()}
    ...
    print(f"\n✅ Song of Songs complete! {total_updates} translations added.")
