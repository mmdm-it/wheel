#!/usr/bin/env python3
"""
Populate Leviticus (27 chapters, ~859 verses) with all 9 languages.
"""
...
CHAPTERS_DIR = PROJECT_ROOT / "data" / "gutenberg" / "chapters" / "LEVI"

NUM_CHAPTERS = 27


def generate_base36_files(start_code, num_chapters):
    """Generate base-36 file mappings."""
...
NAB_LEVITICUS = generate_base36_files("2N", NUM_CHAPTERS)
SPANISH_LEVITICUS = generate_base36_files("2M", NUM_CHAPTERS)
ITALIAN_LEVITICUS = generate_base36_files("2J", NUM_CHAPTERS)
...
def main():
    print("=" * 60)
    print("Populating Leviticus with 9 Languages")
    print("=" * 60)
    
    all_translations = {}
    
    print("\n  Parsing Hebrew (WLC)...")
    all_translations['WLC'] = parse_hebrew_wlc(SOURCES_DIR / "hebrew" / "wlc" / "Lev.xml", "Lev")
    print(f"  ✅ Hebrew WLC: {sum(len(v) for v in all_translations['WLC'].values())} verses" if all_translations['WLC'] else "  ❌ Not found")
    
    print("\n  Parsing Greek (LXX)...")
    all_translations['LXX'] = parse_greek_lxx(SOURCES_DIR / "greek" / "septuagint" / "03.Lev.txt")
    print(f"  ✅ Greek LXX: {sum(len(v) for v in all_translations['LXX'].values())} verses" if all_translations['LXX'] else "  ❌ Not found")
    
    print("\n  Parsing Latin (VUL)...")
    all_translations['VUL'] = parse_latin_vulgate(SOURCES_DIR / "latin" / "clementine" / "src" / "utf8" / "Lv.lat")
    print(f"  ✅ Latin VUL: {sum(len(v) for v in all_translations['VUL'].values())} verses" if all_translations['VUL'] else "  ❌ Not found")
    
    print("\n  Parsing Russian (SYN)...")
    all_translations['SYN'] = parse_russian_synodal(SOURCES_DIR / "russian" / "synodal-77books" / "Синодальный перевод - 77 книг - txt" / "03_lev.txt")
    print(f"  ✅ Russian SYN: {sum(len(v) for v in all_translations['SYN'].values())} verses" if all_translations['SYN'] else "  ❌ Not found")
    
    print("\n  Parsing French (NEO)...")
    all_translations['NEO'] = parse_french_usfm(SOURCES_DIR / "french" / "neo-crampon" / "04-LEVfrancl.usfm")
    print(f"  ✅ French NEO: {sum(len(v) for v in all_translations['NEO'].values())} verses" if all_translations['NEO'] else "  ❌ Not found")
    
    print("\n  Parsing English (NAB)...")
    nab_files = {ch: SOURCES_DIR / "english" / "nab-vatican" / f for ch, f in NAB_LEVITICUS.items()}
    ...
    spanish_files = {ch: SOURCES_DIR / "spanish" / "libro-pueblo-dios" / f for ch, f in SPANISH_LEVITICUS.items()}
    ...
    print("\n  Parsing Italian (CEI)...")
    italian_files = {ch: SOURCES_DIR / "italian" / "vatican" / f for ch, f in ITALIAN_LEVITICUS.items()}
    ...
    print(f"\n✅ Leviticus complete! {total_updates} translations added.")
