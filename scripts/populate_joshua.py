#!/usr/bin/env python3
"""
Populate Joshua (24 chapters, ~658 verses) with all 9 languages.
"""
...
CHAPTERS_DIR = PROJECT_ROOT / "data" / "gutenberg" / "chapters" / "IOSU"
...
# Joshua chapter files (24 chapters)
# NAB: __P5F.HTM (ch1) through __P6C.HTM (ch24) [5F in base36 = 5*36+15 = 195]
# Spanish: __P5E.HTM (ch1) through __P6B.HTM (ch24) [5E = 194]
# Italian: __P58.HTM (ch1) through __P5V.HTM (ch24) [58 = 188]
NAB_JOSHUA = generate_base36_files("5F", NUM_CHAPTERS)
SPANISH_JOSHUA = generate_base36_files("5E", NUM_CHAPTERS)
ITALIAN_JOSHUA = generate_base36_files("58", NUM_CHAPTERS)
...
def main():
    print("=" * 60)
    print("Populating Joshua with 9 Languages")
    print("=" * 60)
    ...
    # Hebrew WLC
    print("\n  Parsing Hebrew (WLC)...")
    all_translations['WLC'] = parse_hebrew_wlc(SOURCES_DIR / "hebrew" / "wlc" / "Josh.xml", "Josh")
    print(f"  ✅ Hebrew WLC: {sum(len(v) for v in all_translations['WLC'].values())} verses" if all_translations['WLC'] else "  ❌ Not found")
    
    # Greek LXX
    print("\n  Parsing Greek (LXX)...")
    all_translations['LXX'] = parse_greek_lxx(SOURCES_DIR / "greek" / "septuagint" / "06.Jos.txt")
    print(f"  ✅ Greek LXX: {sum(len(v) for v in all_translations['LXX'].values())} verses" if all_translations['LXX'] else "  ❌ Not found")
    
    # Latin Vulgate
    print("\n  Parsing Latin (VUL)...")
    all_translations['VUL'] = parse_latin_vulgate(SOURCES_DIR / "latin" / "clementine" / "src" / "utf8" / "Jos.lat")
    print(f"  ✅ Latin VUL: {sum(len(v) for v in all_translations['VUL'].values())} verses" if all_translations['VUL'] else "  ❌ Not found")
    
    # Russian Synodal
    print("\n  Parsing Russian (SYN)...")
    all_translations['SYN'] = parse_russian_synodal(SOURCES_DIR / "russian" / "synodal-77books" / "Синодальный перевод - 77 книг - txt" / "06_isnav.txt")
    print(f"  ✅ Russian SYN: {sum(len(v) for v in all_translations['SYN'].values())} verses" if all_translations['SYN'] else "  ❌ Not found")
    
    # French néo-Crampon
    print("\n  Parsing French (NEO)...")
    all_translations['NEO'] = parse_french_usfm(SOURCES_DIR / "french" / "neo-crampon" / "07-JOSfrancl.usfm")
    ...
    nab_files = {ch: nab_dir / fname for ch, fname in NAB_JOSHUA.items()}
    ...
    spanish_files = {ch: spanish_dir / fname for ch, fname in SPANISH_JOSHUA.items()}
    ...
    italian_files = {ch: italian_dir / fname for ch, fname in ITALIAN_JOSHUA.items()}
    ...
    print(f"\n✅ Joshua complete! {total_updates} translations added.")
