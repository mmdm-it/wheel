...
/**
 * Update Genesis 1 with complete Latin Vulgate text and recalculate word counts
 */

const fs = require('fs');
const path = require('path');

// Complete Latin Vulgate text for Genesis 1 (Clementine Vulgate from sacred-texts.com)
const GENESIS_1_LATIN = {
...
// Count words in a text (simple space-based split)
...
// Main
const chapterPath = path.join(__dirname, '..', 'data/gutenberg/chapters/GENE/001.json');
const chapter = JSON.parse(fs.readFileSync(chapterPath, 'utf8'));

let updated = 0;

for (const [verseNum, latinText] of Object.entries(GENESIS_1_LATIN)) {
    if (chapter.verses[verseNum]) {
        chapter.verses[verseNum].latin = latinText;
        chapter.verses[verseNum].word_count = countWords(latinText);
        updated++;
        console.log(`Verse ${verseNum}: ${countWords(latinText)} words`);
    }
}

fs.writeFileSync(chapterPath, JSON.stringify(chapter, null, 2));
console.log(`\n✅ Updated ${updated} verses with Latin text and word counts`);
