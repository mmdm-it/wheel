#!/usr/bin/env python3
"""
Populate 1 Kings (22 chapters, ~816 verses) with all 9 languages.
"""
...
CHAPTERS_DIR = PROJECT_ROOT / "data" / "gutenberg" / "chapters" / "III_REG"
...
# 1 Kings chapter files (22 chapters)
# NAB: __P8F.HTM (ch1) - 8F in base36 = 8*36+15 = 303
# Spanish: __P89.HTM (ch1) - 89 = 8*36+9 = 297
# Italian: __P84.HTM (ch1) - 84 = 8*36+4 = 292
NAB_1KINGS = generate_base36_files("8F", NUM_CHAPTERS)
SPANISH_1KINGS = generate_base36_files("89", NUM_CHAPTERS)
ITALIAN_1KINGS = generate_base36_files("84", NUM_CHAPTERS)
...
def main():
    print("=" * 60)
    print("Populating 1 Kings with 9 Languages")
    print("=" * 60)
    ...
    # Hebrew WLC
    print("\n  Parsing Hebrew (WLC)...")
    all_translations['WLC'] = parse_hebrew_wlc(SOURCES_DIR / "hebrew" / "wlc" / "1Kgs.xml", "1Kgs")
    print(f"  ✅ Hebrew WLC: {sum(len(v) for v in all_translations['WLC'].values())} verses" if all_translations['WLC'] else "  ❌ Not found")
    
    # Greek LXX
    print("\n  Parsing Greek (LXX)...")
    all_translations['LXX'] = parse_greek_lxx(SOURCES_DIR / "greek" / "septuagint" / "11.1Ki.txt")
    print(f"  ✅ Greek LXX: {sum(len(v) for v in all_translations['LXX'].values())} verses" if all_translations['LXX'] else "  ❌ Not found")
    
    # Latin Vulgate
    print("\n  Parsing Latin (VUL)...")
    all_translations['VUL'] = parse_latin_vulgate(SOURCES_DIR / "latin" / "clementine" / "src" / "utf8" / "3Rg.lat")
    print(f"  ✅ Latin VUL: {sum(len(v) for v in all_translations['VUL'].values())} verses" if all_translations['VUL'] else "  ❌ Not found")
    
    # Russian Synodal
    print("\n  Parsing Russian (SYN)...")
    all_translations['SYN'] = parse_russian_synodal(SOURCES_DIR / "russian" / "synodal-77books" / "Синодальный перевод - 77 книг - txt" / "11_3tsar.txt")
    print(f"  ✅ Russian SYN: {sum(len(v) for v in all_translations['SYN'].values())} verses" if all_translations['SYN'] else "  ❌ Not found")
    
    # French néo-Crampon
    print("\n  Parsing French (NEO)...")
    all_translations['NEO'] = parse_french_usfm(SOURCES_DIR / "french" / "neo-crampon" / "12-1KIfrancl.usfm")
    ...
    nab_files = {ch: nab_dir / fname for ch, fname in NAB_1KINGS.items()}
    ...
    spanish_files = {ch: spanish_dir / fname for ch, fname in SPANISH_1KINGS.items()}
    ...
    italian_files = {ch: italian_dir / fname for ch, fname in ITALIAN_1KINGS.items()}
    ...
    print(f"\n✅ 1 Kings complete! {total_updates} translations added.")
