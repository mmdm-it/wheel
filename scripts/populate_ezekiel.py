#!/usr/bin/env python3
"""
Populate Ezekiel (48 chapters, ~1273 verses) with all 9 languages.
"""
...
CHAPTERS_DIR = PROJECT_ROOT / "data" / "gutenberg" / "chapters" / "EZE"
...
# Ezekiel chapter files (48 chapters)
# NAB: __PR9.HTM (ch1) - R9 = 27*36+9 = 981
# Spanish: __PCZ.HTM (ch1) - CZ = 12*36+35 = 467
# Italian: __PQ4.HTM (ch1) - Q4 = 26*36+4 = 940
NAB_EZE = generate_base36_files("R9", NUM_CHAPTERS)
SPANISH_EZE = generate_base36_files("CZ", NUM_CHAPTERS)
ITALIAN_EZE = generate_base36_files("Q4", NUM_CHAPTERS)
...
def main():
    print("=" * 60)
    print("Populating Ezekiel with 9 Languages")
    print("=" * 60)
    ...
    # Hebrew WLC
    print("\n  Parsing Hebrew (WLC)...")
    all_translations['WLC'] = parse_hebrew_wlc(SOURCES_DIR / "hebrew" / "wlc" / "Ezek.xml", "Ezek")
    print(f"  ✅ Hebrew WLC: {sum(len(v) for v in all_translations['WLC'].values())} verses" if all_translations['WLC'] else "  ❌ Not found")
    
    # Greek LXX
    print("\n  Parsing Greek (LXX)...")
    all_translations['LXX'] = parse_greek_lxx(SOURCES_DIR / "greek" / "septuagint" / "47.Eze.txt")
    print(f"  ✅ Greek LXX: {sum(len(v) for v in all_translations['LXX'].values())} verses" if all_translations['LXX'] else "  ❌ Not found")
    
    # Latin Vulgate
    print("\n  Parsing Latin (VUL)...")
    all_translations['VUL'] = parse_latin_vulgate(SOURCES_DIR / "latin" / "clementine" / "src" / "utf8" / "Ez.lat")
    print(f"  ✅ Latin VUL: {sum(len(v) for v in all_translations['VUL'].values())} verses" if all_translations['VUL'] else "  ❌ Not found")
    
    # Russian Synodal
    print("\n  Parsing Russian (SYN)...")
    all_translations['SYN'] = parse_russian_synodal(SOURCES_DIR / "russian" / "synodal-77books" / "Синодальный перевод - 77 книг - txt" / "33_iez.txt")
    print(f"  ✅ Russian SYN: {sum(len(v) for v in all_translations['SYN'].values())} verses" if all_translations['SYN'] else "  ❌ Not found")
    
    # French Neo-Crampon
    print("\n  Parsing French (NEO)...")
    all_translations['NEO'] = parse_french_usfm(SOURCES_DIR / "french" / "neo-crampon" / "27-EZKfrancl.usfm")
    ...
    nab_files = {ch: nab_dir / fname for ch, fname in NAB_EZE.items()}
    ...
    spanish_files = {ch: spanish_dir / fname for ch, fname in SPANISH_EZE.items()}
    ...
    italian_files = {ch: italian_dir / fname for ch, fname in ITALIAN_EZE.items()}
    ...
    print(f"\n✅ Ezekiel complete! {total_updates} translations added.")
