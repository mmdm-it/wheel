#!/usr/bin/env python3
"""
Populate Ecclesiastes (12 chapters, ~222 verses) with all 9 languages.
"""
...
CHAPTERS_DIR = PROJECT_ROOT / "data" / "gutenberg" / "chapters" / "ECCLE"
...
# Ecclesiastes chapter files (12 chapters)
# NAB: __PKW.HTM (ch1) - KW in base36 = 20*36+32 = 752
# Spanish: __PMQ.HTM (ch1) - MQ = 22*36+26 = 818
# Italian: __PK1.HTM (ch1) - K1 = 20*36+1 = 721
NAB_ECCL = generate_base36_files("KW", NUM_CHAPTERS)
SPANISH_ECCL = generate_base36_files("MQ", NUM_CHAPTERS)
ITALIAN_ECCL = generate_base36_files("K1", NUM_CHAPTERS)
...
def main():
    print("=" * 60)
    print("Populating Ecclesiastes with 9 Languages")
    print("=" * 60)
    ...
    # Hebrew WLC
    print("\n  Parsing Hebrew (WLC)...")
    all_translations['WLC'] = parse_hebrew_wlc(SOURCES_DIR / "hebrew" / "wlc" / "Eccl.xml", "Eccl")
    print(f"  ✅ Hebrew WLC: {sum(len(v) for v in all_translations['WLC'].values())} verses" if all_translations['WLC'] else "  ❌ Not found")
    
    # Greek LXX
    print("\n  Parsing Greek (LXX)...")
    all_translations['LXX'] = parse_greek_lxx(SOURCES_DIR / "greek" / "septuagint" / "20.Ecc.txt")
    print(f"  ✅ Greek LXX: {sum(len(v) for v in all_translations['LXX'].values())} verses" if all_translations['LXX'] else "  ❌ Not found")
    
    # Latin Vulgate
    print("\n  Parsing Latin (VUL)...")
    all_translations['VUL'] = parse_latin_vulgate(SOURCES_DIR / "latin" / "clementine" / "src" / "utf8" / "Ecl.lat")
    print(f"  ✅ Latin VUL: {sum(len(v) for v in all_translations['VUL'].values())} verses" if all_translations['VUL'] else "  ❌ Not found")
    
    # Russian Synodal
    print("\n  Parsing Russian (SYN)...")
    all_translations['SYN'] = parse_russian_synodal(SOURCES_DIR / "russian" / "synodal-77books" / "Синодальный перевод - 77 книг - txt" / "24_ekk.txt")
    print(f"  ✅ Russian SYN: {sum(len(v) for v in all_translations['SYN'].values())} verses" if all_translations['SYN'] else "  ❌ Not found")
    
    # French néo-Crampon
    print("\n  Parsing French (NEO)...")
    all_translations['NEO'] = parse_french_usfm(SOURCES_DIR / "french" / "neo-crampon" / "22-ECCfrancl.usfm")
    ...
    nab_files = {ch: nab_dir / fname for ch, fname in NAB_ECCL.items()}
    ...
    spanish_files = {ch: spanish_dir / fname for ch, fname in SPANISH_ECCL.items()}
    ...
    italian_files = {ch: italian_dir / fname for ch, fname in ITALIAN_ECCL.items()}
    ...
    print(f"\n✅ Ecclesiastes complete! {total_updates} translations added.")
