#!/usr/bin/env python3
"""
Populate Judges (21 chapters, ~618 verses) with all 9 languages.
"""
...
CHAPTERS_DIR = PROJECT_ROOT / "data" / "gutenberg" / "chapters" / "IUDI"
...
# Judges chapter files (21 chapters)
# NAB: __P64.HTM (ch1) - 64 in base36 = 6*36+4 = 220
# Spanish: __P63.HTM (ch1) - 63 = 219
# Italian: __P5W.HTM (ch1) - 5W = 5*36+32 = 212
NAB_JUDGES = generate_base36_files("64", NUM_CHAPTERS)
SPANISH_JUDGES = generate_base36_files("63", NUM_CHAPTERS)
ITALIAN_JUDGES = generate_base36_files("5W", NUM_CHAPTERS)
...
def main():
    print("=" * 60)
    print("Populating Judges with 9 Languages")
    print("=" * 60)
    ...
    # Hebrew WLC
    print("\n  Parsing Hebrew (WLC)...")
    all_translations['WLC'] = parse_hebrew_wlc(SOURCES_DIR / "hebrew" / "wlc" / "Judg.xml", "Judg")
    print(f"  ✅ Hebrew WLC: {sum(len(v) for v in all_translations['WLC'].values())} verses" if all_translations['WLC'] else "  ❌ Not found")
    
    # Greek LXX
    print("\n  Parsing Greek (LXX)...")
    all_translations['LXX'] = parse_greek_lxx(SOURCES_DIR / "greek" / "septuagint" / "07.Jdg.txt")
    print(f"  ✅ Greek LXX: {sum(len(v) for v in all_translations['LXX'].values())} verses" if all_translations['LXX'] else "  ❌ Not found")
    
    # Latin Vulgate
    print("\n  Parsing Latin (VUL)...")
    all_translations['VUL'] = parse_latin_vulgate(SOURCES_DIR / "latin" / "clementine" / "src" / "utf8" / "Jdc.lat")
    print(f"  ✅ Latin VUL: {sum(len(v) for v in all_translations['VUL'].values())} verses" if all_translations['VUL'] else "  ❌ Not found")
    
    # Russian Synodal
    print("\n  Parsing Russian (SYN)...")
    all_translations['SYN'] = parse_russian_synodal(SOURCES_DIR / "russian" / "synodal-77books" / "Синодальный перевод - 77 книг - txt" / "07_sud.txt")
    print(f"  ✅ Russian SYN: {sum(len(v) for v in all_translations['SYN'].values())} verses" if all_translations['SYN'] else "  ❌ Not found")
    
    # French néo-Crampon
    print("\n  Parsing French (NEO)...")
    all_translations['NEO'] = parse_french_usfm(SOURCES_DIR / "french" / "neo-crampon" / "08-JDGfrancl.usfm")
    ...
    nab_files = {ch: nab_dir / fname for ch, fname in NAB_JUDGES.items()}
    ...
    spanish_files = {ch: spanish_dir / fname for ch, fname in SPANISH_JUDGES.items()}
    ...
    italian_files = {ch: italian_dir / fname for ch, fname in ITALIAN_JUDGES.items()}
    ...
    print(f"\n✅ Judges complete! {total_updates} translations added.")
