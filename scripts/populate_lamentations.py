#!/usr/bin/env python3
"""
Populate Lamentations (5 chapters, ~154 verses) with all 9 languages.
"""
...
CHAPTERS_DIR = PROJECT_ROOT / "data" / "gutenberg" / "chapters" / "LAME"
...
def generate_base36_files(start_code, num_chapters):
    ...
    if len(start_code) == 1:
        ...
        # 3-char code like 'QW' starting after 'P'
        start = chars.index(start_code[0].upper()) * 36 + chars.index(start_code[1].upper())
...
# Lamentations chapter files (5 chapters)
# NAB: __PQW.HTM (ch1) - QW = 26*36+32 = 968
# Spanish: __PN2.HTM (ch1) - N2 = 23*36+2 = 830
# Italian: __PPT.HTM (ch1) - PT = 25*36+29 = 929
NAB_LAM = generate_base36_files("QW", NUM_CHAPTERS)
SPANISH_LAM = generate_base36_files("N2", NUM_CHAPTERS)
ITALIAN_LAM = generate_base36_files("PT", NUM_CHAPTERS)
...
def main():
    print("=" * 60)
    print("Populating Lamentations with 9 Languages")
    print("=" * 60)
    ...
    # Hebrew WLC
    print("\n  Parsing Hebrew (WLC)...")
    all_translations['WLC'] = parse_hebrew_wlc(SOURCES_DIR / "hebrew" / "wlc" / "Lam.xml", "Lam")
    print(f"  ✅ Hebrew WLC: {sum(len(v) for v in all_translations['WLC'].values())} verses" if all_translations['WLC'] else "  ❌ Not found")
    
    # Greek LXX
    print("\n  Parsing Greek (LXX)...")
    all_translations['LXX'] = parse_greek_lxx(SOURCES_DIR / "greek" / "septuagint" / "45.Lam.txt")
    print(f"  ✅ Greek LXX: {sum(len(v) for v in all_translations['LXX'].values())} verses" if all_translations['LXX'] else "  ❌ Not found")
    
    # Latin Vulgate
    print("\n  Parsing Latin (VUL)...")
    all_translations['VUL'] = parse_latin_vulgate(SOURCES_DIR / "latin" / "clementine" / "src" / "utf8" / "Lam.lat")
    print(f"  ✅ Latin VUL: {sum(len(v) for v in all_translations['VUL'].values())} verses" if all_translations['VUL'] else "  ❌ Not found")
    
    # Russian Synodal
    print("\n  Parsing Russian (SYN)...")
    all_translations['SYN'] = parse_russian_synodal(SOURCES_DIR / "russian" / "synodal-77books" / "Синодальный перевод - 77 книг - txt" / "30_plach.txt")
    print(f"  ✅ Russian SYN: {sum(len(v) for v in all_translations['SYN'].values())} verses" if all_translations['SYN'] else "  ❌ Not found")
    
    # French Neo-Crampon
    print("\n  Parsing French (NEO)...")
    all_translations['NEO'] = parse_french_usfm(SOURCES_DIR / "french" / "neo-crampon" / "26-LAMfrancl.usfm")
    ...
    nab_files = {ch: nab_dir / fname for ch, fname in NAB_LAM.items()}
    ...
    spanish_files = {ch: spanish_dir / fname for ch, fname in SPANISH_LAM.items()}
    ...
    italian_files = {ch: italian_dir / fname for ch, fname in ITALIAN_LAM.items()}
    ...
    print(f"\n✅ Lamentations complete! {total_updates} translations added.")
