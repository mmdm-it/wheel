#!/usr/bin/env python3
"""
Populate 2 Kings (25 chapters, ~719 verses) with all 9 languages.
"""
...
CHAPTERS_DIR = PROJECT_ROOT / "data" / "gutenberg" / "chapters" / "IV_REG"
...
# 2 Kings chapter files (25 chapters)
# NAB: __P91.HTM (ch1) - 91 in base36 = 9*36+1 = 325
# Spanish: __P8V.HTM (ch1) - 8V = 8*36+31 = 319
# Italian: __P8Q.HTM (ch1) - 8Q = 8*36+26 = 314
NAB_2KINGS = generate_base36_files("91", NUM_CHAPTERS)
SPANISH_2KINGS = generate_base36_files("8V", NUM_CHAPTERS)
ITALIAN_2KINGS = generate_base36_files("8Q", NUM_CHAPTERS)
...
def main():
    print("=" * 60)
    print("Populating 2 Kings with 9 Languages")
    print("=" * 60)
    ...
    # Hebrew WLC
    print("\n  Parsing Hebrew (WLC)...")
    all_translations['WLC'] = parse_hebrew_wlc(SOURCES_DIR / "hebrew" / "wlc" / "2Kgs.xml", "2Kgs")
    print(f"  ✅ Hebrew WLC: {sum(len(v) for v in all_translations['WLC'].values())} verses" if all_translations['WLC'] else "  ❌ Not found")
    
    # Greek LXX
    print("\n  Parsing Greek (LXX)...")
    all_translations['LXX'] = parse_greek_lxx(SOURCES_DIR / "greek" / "septuagint" / "12.2Ki.txt")
    print(f"  ✅ Greek LXX: {sum(len(v) for v in all_translations['LXX'].values())} verses" if all_translations['LXX'] else "  ❌ Not found")
    
    # Latin Vulgate
    print("\n  Parsing Latin (VUL)...")
    all_translations['VUL'] = parse_latin_vulgate(SOURCES_DIR / "latin" / "clementine" / "src" / "utf8" / "4Rg.lat")
    print(f"  ✅ Latin VUL: {sum(len(v) for v in all_translations['VUL'].values())} verses" if all_translations['VUL'] else "  ❌ Not found")
    
    # Russian Synodal
    print("\n  Parsing Russian (SYN)...")
    all_translations['SYN'] = parse_russian_synodal(SOURCES_DIR / "russian" / "synodal-77books" / "Синодальный перевод - 77 книг - txt" / "12_4tsar.txt")
    print(f"  ✅ Russian SYN: {sum(len(v) for v in all_translations['SYN'].values())} verses" if all_translations['SYN'] else "  ❌ Not found")
    
    # French néo-Crampon
    print("\n  Parsing French (NEO)...")
    all_translations['NEO'] = parse_french_usfm(SOURCES_DIR / "french" / "neo-crampon" / "13-2KIfrancl.usfm")
    ...
    nab_files = {ch: nab_dir / fname for ch, fname in NAB_2KINGS.items()}
    ...
    spanish_files = {ch: spanish_dir / fname for ch, fname in SPANISH_2KINGS.items()}
    ...
    italian_files = {ch: italian_dir / fname for ch, fname in ITALIAN_2KINGS.items()}
    ...
    print(f"\n✅ 2 Kings complete! {total_updates} translations added.")
