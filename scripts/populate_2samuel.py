#!/usr/bin/env python3
"""
Populate 2 Samuel (24 chapters, ~695 verses) with all 9 languages.
"""
...
CHAPTERS_DIR = PROJECT_ROOT / "data" / "gutenberg" / "chapters" / "II_SAM"
...
# 2 Samuel chapter files (24 chapters)
# NAB: __P7Q.HTM (ch1) - 7Q in base36 = 7*36+26 = 278
# Spanish: __P7K.HTM (ch1) - 7K = 7*36+20 = 272
# Italian: __P7G.HTM (ch1) - 7G = 7*36+16 = 268
NAB_2SAM = generate_base36_files("7Q", NUM_CHAPTERS)
SPANISH_2SAM = generate_base36_files("7K", NUM_CHAPTERS)
ITALIAN_2SAM = generate_base36_files("7G", NUM_CHAPTERS)
...
def main():
    print("=" * 60)
    print("Populating 2 Samuel with 9 Languages")
    print("=" * 60)
    ...
    # Hebrew WLC
    print("\n  Parsing Hebrew (WLC)...")
    all_translations['WLC'] = parse_hebrew_wlc(SOURCES_DIR / "hebrew" / "wlc" / "2Sam.xml", "2Sam")
    print(f"  ✅ Hebrew WLC: {sum(len(v) for v in all_translations['WLC'].values())} verses" if all_translations['WLC'] else "  ❌ Not found")
    
    # Greek LXX
    print("\n  Parsing Greek (LXX)...")
    all_translations['LXX'] = parse_greek_lxx(SOURCES_DIR / "greek" / "septuagint" / "10.2Sa.txt")
    print(f"  ✅ Greek LXX: {sum(len(v) for v in all_translations['LXX'].values())} verses" if all_translations['LXX'] else "  ❌ Not found")
    
    # Latin Vulgate
    print("\n  Parsing Latin (VUL)...")
    all_translations['VUL'] = parse_latin_vulgate(SOURCES_DIR / "latin" / "clementine" / "src" / "utf8" / "2Rg.lat")
    print(f"  ✅ Latin VUL: {sum(len(v) for v in all_translations['VUL'].values())} verses" if all_translations['VUL'] else "  ❌ Not found")
    
    # Russian Synodal
    print("\n  Parsing Russian (SYN)...")
    all_translations['SYN'] = parse_russian_synodal(SOURCES_DIR / "russian" / "synodal-77books" / "Синодальный перевод - 77 книг - txt" / "10_2tsar.txt")
    print(f"  ✅ Russian SYN: {sum(len(v) for v in all_translations['SYN'].values())} verses" if all_translations['SYN'] else "  ❌ Not found")
    
    # French néo-Crampon
    print("\n  Parsing French (NEO)...")
    all_translations['NEO'] = parse_french_usfm(SOURCES_DIR / "french" / "neo-crampon" / "11-2SAfrancl.usfm")
    ...
    nab_files = {ch: nab_dir / fname for ch, fname in NAB_2SAM.items()}
    ...
    spanish_files = {ch: spanish_dir / fname for ch, fname in SPANISH_2SAM.items()}
    ...
    italian_files = {ch: italian_dir / fname for ch, fname in ITALIAN_2SAM.items()}
    ...
    print(f"\n✅ 2 Samuel complete! {total_updates} translations added.")
