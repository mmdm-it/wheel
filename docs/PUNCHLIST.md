...
- [ ] [DATA] **Esther is 15/16 empty**: chapters 1–7 and 9–16 ship as verse-less stubs (only ch. 8 has text; found 2026-07-20 via the Moto G near-crash). ~350 verses missing entirely — the B.1 Clementine fill evidently broke on Vulgate Esther's odd versification. Needs a B.1-style refill from wheel-v0/sources/latin. The manifest's verse_counts are non-zero, so the verse chain synthesizes links whose text can never arrive — refill fixes both.
