...
// Render markets as buttons below central
function renderMarkets() {
    marketsGroup.innerHTML = '';
    const markets = Object.keys(data.MMdM.markets);
    markets.forEach((market, index) => {
        const x = (index - (markets.length - 1) / 2) * 300;
        const y = -100;
        const width = 250;
        const height = 40;
        ...
        text.textContent = market;
        g.appendChild(text);
        marketsGroup.appendChild(g);

        // Add listener to rect
        rect.addEventListener('mouseover', () => {
            if (activeType === 'market' && activePath[0] === market) {
                console.log(`Market ${market} already active, skipping`);
                return;
            }
            console.log(`Activating market: ${market}`);
            if (activeType === 'model') {
                revertCentral();
            }
            activeType = 'market';
            activePath = [market];
            resetView(false);
            renderNextLevel('market', market, 0);
            renderPathLines();
        });
    });
}
...
function showModelInfo(modelFullKey) {
    ...
    let endY = 80;
    let endW = 200;
    let endH = 63; // Approximate 240 * (126/400) ≈ 75.6

    let duration = 750; // ms, slowed down for debugging
...
function revertCentral() {
    centralGroup.innerHTML = `
        ...
        <image xlink:href="./assets/catalog_logo.png" x="-70" y="-32" width="140" height="44" />
    `;
...
function renderManufacturers(market, selectedManufacturerKey = null) {
    ...
    const angleSpread = Math.PI / 30; // ~4.29 degrees in radians
...
function renderNextLevel(type, name, angle) {
    ...
    // Control markets visibility: hide when models are revealed
    if (type === 'cylinder' || type === 'model') {
